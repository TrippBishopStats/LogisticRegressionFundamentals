...
make sense? These considerations are more important than numerical associations
in your data.
:::
...
## In Summary
The scenarios above highlight why it is so important to identify and gather data
on confounders before conducting an experiment or observational study. If left
uncontrolled, the confounders could bias the estimated treatment effect,
resulting in spurious conclusions about the relationship under investigation.
Even if a confounder has a parameter coefficient that is not reliably different
to zero, it should be included in the effects model.
